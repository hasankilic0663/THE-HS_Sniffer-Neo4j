...
neo4jGraph = Graph(auth=("neo4j", "agprogramlama"))

def add_dbms(pkt):

    if "TCP" in pkt:
        firstNode = Node("Host", name=pkt[IP].src, port=pkt[IP].sport)
        secondNode = Node("Host", name=pkt[IP].dst, port=pkt[IP].dport)

        SENDtcp = Relationship.type("TCP")
        tcp_rel = SENDtcp(firstNode, secondNode)
        neo4jGraph.merge(tcp_rel, "Host", ("name", "port"))

    if "UDP" in pkt:
        firstNode = Node("Host", name=pkt[1].src, port=pkt[1].sport)

        secondNode = Node("Host", name=pkt[1].dst, port=pkt[1].dport)

        SENDudp = Relationship.type("UDP")
        udp_rel = SENDudp(firstNode, secondNode)
        neo4jGraph.merge(udp_rel, "Host", ("name", "port"))

    """if "ICMP" in pkt:
        firstNode = Node("Host", name=pkt[IP].src)

        secondNode = Node("Host", name=pkt[IP].dst)
    ...
    """elif "ARP" in pkt:
        ...
        firstNode = Node("Host", name=pkt[ARP].psrc)

        # the fourth element in the packet object list is the sender address
        secondNode = Node("Host", name=pkt[ARP].pdst)
        ...
        neo4jGraph.merge(SENDarp(firstNode, secondNode))"""
